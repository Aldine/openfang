...
pub async fn run_agent_loop(
    ...
    let memories = if disable_memory {
        info!(agent = %manifest.name, elapsed_ms = request_started_at.elapsed().as_millis(), "runtime.memory_disabled");
        vec![]
    ...
    if !memories.is_empty() {
        let prompt_ctx = crate::prompt_builder::PromptContext {
            recalled_memories: memories.clone(),
            memory_item_limit: 5,
            ..Default::default()
        };
        system_prompt.push_str("\n\n");
        system_prompt.push_str(&crate::prompt_builder::build_memory_section(&prompt_ctx));
    }
    ...
    for iteration in 0..max_iterations {
        ...
        match response.stop_reason {
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if disable_memory {
                    debug!(agent = %manifest.name, "Skipping conversation memory persistence");
                } else if let Some(emb) = embedding_driver {
...
pub async fn run_agent_loop_streaming(
    ...
    // Recall relevant memories — prefer vector similarity search when embedding driver is available
    let memories = if disable_memory {
        info!(agent = %manifest.name, elapsed_ms = request_started_at.elapsed().as_millis(), "runtime.memory_disabled_streaming");
        vec![]
    } else if let Some(emb) = embedding_driver {
        match emb.embed_one(user_message).await {
            Ok(query_vec) => {
    ...
    for iteration in 0..max_iterations {
        ...
        info!(
            ...
            iteration,
            elapsed_ms = request_started_at.elapsed().as_millis(),
            "runtime.model_call_started_streaming"
        );

        // Stream LLM call with retry, error classification, and circuit breaker
        let provider_name = manifest.model.provider.as_str();
        let (forward_tx, mut forward_rx) = mpsc::channel(128);
        ...
        let forwarder = tokio::spawn(async move {
            ...
            while let Some(event) = forward_rx.recv().await {
                ...
                if stream_tx_clone.send(event).await.is_err() {
                    break;
                }
            }
        ...
        match response.stop_reason {
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                info!(
                    ...
                    elapsed_ms = request_started_at.elapsed().as_millis(),
                    response_chars = final_response.len(),
                    "runtime.final_token_received_streaming"
                );
                session.messages.push(Message::assistant(text));
                ...
                if disable_memory {
                    debug!(agent = %manifest.name, "Skipping conversation memory persistence (streaming)");
                } else if let Some(emb) = embedding_driver {
                    match emb.embed_one(&interaction_text).await {
                        Ok(vec) => {
                            let _ = memory
...
mod tests {
    ...
    struct ExactTextDriver {
        text: String,
    }

    #[async_trait]
    impl LlmDriver for ExactTextDriver {
        async fn complete(
            &self,
            _request: CompletionRequest,
        ) -> Result<CompletionResponse, LlmError> {
    ...
    struct ChunkedNoReplyStreamDriver;

    #[async_trait]
    impl LlmDriver for ChunkedNoReplyStreamDriver {
        async fn complete(
            &self,
            _request: CompletionRequest,
        ) -> Result<CompletionResponse, LlmError> {
    ...
    struct ToolThenNoReplyDriver {
        call_count: AtomicU32,
    }

    impl ToolThenNoReplyDriver {
        fn new() -> Self {
            Self {
                call_count: AtomicU32::new(0),
            }
    ...
    #[async_trait]
    impl LlmDriver for ToolThenNoReplyDriver {
        async fn complete(
            &self,
            _request: CompletionRequest,
        ) -> Result<CompletionResponse, LlmError> {
    ...
    fn assistant_text_messages(session: &Session) -> Vec<String> {
        session
            .messages
            .iter()
    ...
    fn collect_stream_text(events: &[StreamEvent]) -> String {
        events
            .iter()
    ...
    #[tokio::test]
    async fn no_reply_non_streaming_completes_silently() {
        let memory = openfang_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = openfang_types::agent::AgentId::new();
        let mut session = openfang_memory::session::Session {
            id: openfang_types::agent::SessionId::new(),
        ...
        let manifest = test_manifest();
        let driver: Arc<dyn LlmDriver> = Arc::new(ExactTextDriver {
            text: "NO_REPLY".to_string(),
        });
        ...
        .await
        .expect("NO_REPLY loop should complete");

        assert!(result.silent, "expected silent completion");
        assert!(result.response.is_empty(), "expected no visible response");
        assert!(assistant_text_messages(&session).is_empty());
    }

    #[tokio::test]
    async fn no_reply_streaming_completes_silently() {
        let memory = openfang_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = openfang_types::agent::AgentId::new();
        let mut session = openfang_memory::session::Session {
            id: openfang_types::agent::SessionId::new(),
        ...
        let manifest = test_manifest();
        let driver: Arc<dyn LlmDriver> = Arc::new(ChunkedNoReplyStreamDriver);
        ...
        .await
        .expect("streaming NO_REPLY loop should complete");
        ...
        assert!(result.silent, "expected silent completion");
        assert!(result.response.is_empty(), "expected no visible response");
        assert!(collect_stream_text(&events).trim().is_empty());
        assert!(assistant_text_messages(&session).is_empty());
    }

    #[tokio::test]
    async fn no_reply_requires_exact_token() {
        let memory = openfang_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = openfang_types::agent::AgentId::new();
        let mut session = openfang_memory::session::Session {
            id: openfang_types::agent::SessionId::new(),
        ...
        let manifest = test_manifest();
        let driver: Arc<dyn LlmDriver> = Arc::new(ExactTextDriver {
            text: "NO_REPLY because no response is needed".to_string(),
        });
        ...
        .await
        .expect("non-exact NO_REPLY loop should complete");

        assert!(!result.silent, "non-exact token must not be silent");
        assert_eq!(assistant_text_messages(&session).len(), 1);
        assert_eq!(result.response, "NO_REPLY because no response is needed");
    }

    #[tokio::test]
    async fn no_reply_after_tool_call_completes_silently() {
        let memory = openfang_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = openfang_types::agent::AgentId::new();
        let mut session = openfang_memory::session::Session {
            id: openfang_types::agent::SessionId::new(),
        ...
        let manifest = test_manifest();
        let driver: Arc<dyn LlmDriver> = Arc::new(ToolThenNoReplyDriver::new());

        let result = run_agent_loop(
            &manifest,
            "use a tool then stay silent",
            &mut session,
            &memory,
            driver,
            &[],
        ...
        .await
        .expect("tool then NO_REPLY loop should complete");

        assert!(result.silent, "expected silent completion after tool call");
        assert!(result.response.is_empty(), "expected no visible response");
        assert!(assistant_text_messages(&session).is_empty());
    }

    #[tokio::test]
    async fn no_reply_records_metrics_without_emitting_message() {
        let memory = openfang_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = openfang_types::agent::AgentId::new();
        let mut session = openfang_memory::session::Session {
            id: openfang_types::agent::SessionId::new(),
