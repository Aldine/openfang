...
5. **Configured API port availability** -- if daemon is not running, checks whether `api_listen` (default `127.0.0.1:50051`) is free
...
- Opens the daemon URL (e.g. `http://127.0.0.1:50051/`) in the system browser.
...
openfang config set api_listen "0.0.0.0:50051"
```
...
1. **Read `daemon.json`:** On startup, the daemon writes `~/.openfang/daemon.json` containing the listen address (e.g. `127.0.0.1:50051`). The CLI reads this file to learn where the daemon is.
