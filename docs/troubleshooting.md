...
curl http://127.0.0.1:50051/api/health/detail  # Requires auth
```
...
- Port already in use: change the port mapping `-p 3001:50051`
...
# Test the provider
curl http://127.0.0.1:50051/api/providers/groq/test -X POST
```
...
```bash
curl http://127.0.0.1:50051/api/models
```
...
```bash
curl http://127.0.0.1:50051/api/models/aliases
```
...
```bash
curl -X POST http://127.0.0.1:50051/api/agents/{id}/stop
```
...
```bash
curl -X POST http://127.0.0.1:50051/api/agents/{id}/session/compact
```
...
```bash
curl -H "Authorization: Bearer your-api-key" http://127.0.0.1:50051/api/agents
```
